...
import { advanceTurn, announce, attemptCombination, beginCombination, beginExchange, cancelCombination, choosePower8Self, confirmMemory, discardDrawn, draw, exchange, newGame, nextRound, power7, power8, power9, takeDiscard } from "@/game/engine";
...
export function GameTable() {
  ...
  const [combinationSize, setCombinationSize] = useState<number | null>(null);
  ...
  const canSelect = (playerId: number) => game.active === 0 && !resolving && (
    (playerId === 0 && ["combination-selection", "exchange", "swap-discard", "power7", "power8-self"].includes(game.phase)) ||
  ...
  const clickCard = (playerId: number, index: number) => {
    ...
    if (game.phase === "combination-selection" && playerId === 0) return setSelected(current => current.includes(index) ? current.filter(value => value !== index) : current.length < (combinationSize ?? 0) ? [...current, index] : current);
    if (game.phase === "exchange" || game.phase === "swap-discard") return setGame(exchange(game, index));
    if (game.phase === "power7") { setResolving(true); setRevealed({ 0: [index] }); return setTimeout(() => { setGame(state => state ? power7(state, index) : state); setRevealed({}); setResolving(false); }, 1100); }
    ...
    if (game.phase === "power9") { setResolving(true); setRevealed({ [playerId]: [index] }); return setTimeout(() => { setGame(state => state ? power9(state, playerId, index) : state); setRevealed({}); setResolving(false); }, 1100); }
  };
  ...
  return <main className="app-shell">
    ...
    <div className={`table phase-${game.phase} active-${game.active} ${roundOver ? "round-reveal" : ""}`}>
      {[2, 1, 3, 0].map(id => <PlayerArea key={id} player={game.players[id]} active={game.active === id && !roundOver} phase={game.phase} reveal={roundOver} revealIndices={revealed[id]} selected={id === 0 ? (game.phase === "power8-other" && game.powerSelf !== null ? [game.powerSelf] : selected) : []} selectable={() => canSelect(id)} onCard={index => clickCard(id, index)}/>)}
    ...
    <ActionPanel state={game} selectionCount={selected.length} combinationSize={combinationSize} resolving={resolving} onMemorize={() => setGame(confirmMemory(game))} onDraw={() => setGame(draw(game))} onExchange={() => { setSelected([]); setGame(beginExchange(game)); }} onDiscard={() => { setSelected([]); setGame(discardDrawn(game)); }} onChooseCombination={size => { setCombinationSize(size); setSelected([]); setGame(beginCombination(game)); }} onConfirmCombination={() => { if (!combinationSize || selected.length !== combinationSize) return; setResolving(true); setRevealed({ 0: selected }); setTimeout(() => { setGame(state => state ? attemptCombination(state, selected) : state); setSelected([]); setCombinationSize(null); setRevealed({}); setResolving(false); }, 1100); }} onCancelCombination={() => { setSelected([]); setCombinationSize(null); setGame(cancelCombination(game)); }} onAnnounce={() => setGame(announce(game))} onContinue={() => setGame(advanceTurn(game))}/>
