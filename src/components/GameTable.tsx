...
import { isDrawnCardVisible } from "@/game/presentation";
import { adrianoOutcome, winners } from "@/game/scoring";
...
export function GameTable() {
  ...
  return <main className="app-shell">
    ...
    <div className={`table phase-${game.phase} active-${game.active}`}>
      ...
      <section className="center">
        ...
        <button type="button" className="pile discard-pile" disabled={game.active !== 0 || game.phase !== "choose" || !game.discard.length} onClick={() => setGame(takeDiscard(game))}>
          {game.discard.length ? <Card key={game.discard.at(-1)!.id} card={game.discard.at(-1)} faceUp/> : <div className="empty-pile">Fosse<br/>vide</div>}<b>FOSSE</b><small>{game.discard.length}</small>
        </button>
        {game.drawn && <div className={`drawn ${isDrawnCardVisible(game) ? "human-draw" : "bot-draw bot-target-${game.active}"}`}><Card card={game.drawn} faceUp={isDrawnCardVisible(game)}/><b>{isDrawnCardVisible(game) ? "PIOCHÉE" : `${game.players[game.active].name} pioche`}</b></div>}
      </section>
    </div>
    ...
    {roundOver && <div className="modal"><div>
      ...
      {game.players.map((player, index) => <p key={player.id}><span>{player.name}</span><strong>+{game.roundScores?.[index]} · {player.total} pts</strong></p>)}
      {game.phase === "round-end" && <button onClick={() => setGame(nextRound(game))}>Manche suivante</button>}<button className="secondary" onClick={() => setGame(newGame())}>Nouvelle partie</button>
    </div></div>}
