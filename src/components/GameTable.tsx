...
export function GameTable() {
  ...
  const canSelect = (playerId:number) => game.active === 0 && ((playerId===0 && ["drawn","swap-discard","power7","power8-self"].includes(game.phase)) || (playerId!==0 && ["power8-other","power9"].includes(game.phase)));
  ...
  return <main className="app-shell"><Scoreboard players={game.players} round={game.round}/><div className="table">
    {[2,1,3,0].map(id=><PlayerArea key={id} player={game.players[id]} active={game.active===id && !roundOver} phase={game.phase} reveal={roundOver} selected={id===0?selected:[]} selectable={()=>canSelect(id)} onCard={i=>clickCard(id,i)}/>)}
    <section className="center"><button type="button" className="pile" disabled={game.active!==0||game.phase!=="choose"} onClick={()=>setGame(draw(game))}><Card/><b>PIOCHE</b><small>{game.deck.length}</small></button><button type="button" className="pile" disabled={game.active!==0||game.phase!=="choose"||!game.discard.length} onClick={()=>setGame(takeDiscard(game))}><Card card={game.discard.at(-1)} faceUp/><b>FOSSE</b><small>{game.discard.length}</small></button>{game.drawn&&<div className="drawn"><Card card={game.drawn} faceUp/><b>PIOCHÉE</b></div>}</section>
